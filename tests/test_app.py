...
def test_get_contacts_empty(client):
    rv = client.get('/friendfile/api/contacts')
...
def test_get_contact_by_id(client, sample_data):
    rv = client.get('/friendfile/api/contacts/1')
    assert rv.status_code == 200
    data = json.loads(rv.data)
...
def test_create_contact(client):
    rv = client.post('/friendfile/api/contacts', json={
...
def test_update_contact(client, sample_data):
    rv = client.put('/friendfile/api/contacts/1', json={
...
def test_delete_contact(client, sample_data):
    rv = client.delete('/friendfile/api/contacts/1')
...
def test_create_contact_missing_required(client):
    rv = client.post('/friendfile/api/contacts', json={"firstName": "Jane"})
...
def test_sql_injection_in_query(client, sample_data):
    rv = client.get('/friendfile/api/contacts?query=John')
    assert rv.status_code == 200
    data = json.loads(rv.data)
    assert len(data['data']) == 1
    assert data['data'][0]['firstName'] == 'John'
...
def test_get_nonexistent_contact(client):
    rv = client.get('/friendfile/api/contacts/999')
    assert rv.status_code == 404
    assert json.loads(rv.data)['message'] == 'Contact not found'

def test_delete_nonexistent_contact(client):
    rv = client.delete('/friendfile/api/contacts/999')
    assert rv.status_code == 404
    assert json.loads(rv.data)['message'] == 'Contact not found'

def test_pagination(client, sample_data):
    rv = client.get('/friendfile/api/contacts?page=1&pageCount=1')
    assert rv.status_code == 200
    data = json.loads(rv.data)
    assert len(data['data']) == 1
...
def test_create_contact_invalid_json(client):
    rv = client.post('/friendfile/api/contacts', data='{"firstName": "Jane"', content_type='application/json')
    assert rv.status_code == 400
...
def test_create_contact_max_length(client):
    long_name = "A" * 51
    rv = client.post('/friendfile/api/contacts', json={"firstName": long_name, "lastName": "Smith"})
    assert rv.status_code == 201
    data = json.loads(rv.data)
